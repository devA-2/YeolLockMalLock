	...
	$.ajax({
		...
		success : function(info){
			if(info.isc == "success"){
				...
				html += "<span>소요 시간은 가장 가까운 배달원의 이동시간을 고려하여 산정되었으며, 배달원 사정에 따라 배송 시간이 늦어질 수 있습니다.<br>"
				html += "배송 비용에는 보관 비용은 산정 되어 있지 않습니다.</span>"
				html += "</div>"
				html += "<div>"
				html += "<label for='message'>배송 메시지</label>"
				html += "<input type='text' id='message' name='message' placeholder='배송 메시지(선택)'>"
				html += "</div>"
				html += "<div>"
				html += "<input type='submit' value='배송하기'>"
				html += "</div>"	
				$('#result').html(html)	
